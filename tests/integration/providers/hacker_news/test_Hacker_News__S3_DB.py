...
from myfeeds_ai.data_feeds.Data_Feeds__S3__Key_Generator                                       import Data_Feeds__S3__Key_Generator, S3_Key__File__Extension
...
class test_Hacker_News__S3_DB(TestCase):
    ...
    def test_raw_data__feed_xml__save(self):
        with self.s3_db_hacker_news as _:
            ...
            year, month, day, hour = _.s3_key_generator.path__for_date_time__now_utc().split('/')
            s3_path                = _.s3_key_generator.s3_path(int(year), int(month), int(day), int(hour), file_id=S3_FILE_NAME__RAW__FEED_XML, extension=S3_Key__File__Extension.JSON)
