...
class test__int__test_Owasp__Files__Top_10(TestCase):
    ...
    def test_taxonomy(self):
        taxonomy = self.owasp_files_top_10.taxonomy(category=self.category)
        assert type(taxonomy) is Schema__RDF__Taxonomy

    def test_rdf_triples(self):
        rdf_triples = self.owasp_files_top_10.rdf_triples(category=self.category)
        assert type(rdf_triples) is Schema__RDF__Triples
        #pprint(rdf_triples.json())
        #pprint(taxonomy)