...
class Hacker_News__File(Hacker_News__File__Now):
    ...
    def path_latest      (self) -> str : return self.hacker_news_storage.path__latest      (file_id = self.latest_file_id() , extension=self.extension)
    ...
    def latest_file_id(self):
        if self.latest_prefix:
            return Safe_Id(f'{self.latest_prefix}__{self.file_id}')
        else:
            return self.file_id

    def load(self):                                                                             # load file data from 'latest' path
        self.file_data = self.hacker_news_storage.load_from__latest(file_id=self.latest_file_id(), extension=self.extension, content_type=self.content_type)
    ...
    def save(self):                     # todo, refactor with the code in super().save() , make sure we don't need to call str_to_bytes (since that has some performance impact)
        ...
        with self.hacker_news_storage as _:
            ...
            save__path_latest = _.save_to__latest(data=data, file_id=self.latest_file_id(), extension=self.extension, content_type=self.content_type)
