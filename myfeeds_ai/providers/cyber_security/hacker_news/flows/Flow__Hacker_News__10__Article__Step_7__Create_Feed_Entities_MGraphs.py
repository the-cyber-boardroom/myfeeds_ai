...
FLOW__HACKER_NEWS__7__MAX__ARTICLES_TO_PROCESS = 1
...
class Flow__Hacker_News__10__Article__Step_7__Create_Feed_Entities_MGraphs(Type_Safe):
    ...
    max_articles_to_process                  : int = FLOW__HACKER_NEWS__7__MAX__ARTICLES_TO_PROCESS
    output                                   : dict
    ...
    def task__2__create_file_with_feed_text_entities_mgraph(self):
        ...
        file__feed_text_entities              = self.feed_text_entities.file__feed_text_entities             ()
        file__feed_text_entities_titles       = self.feed_text_entities.file__feed_text_entities_titles      ()
        file__feed_text_entities_descriptions = self.feed_text_entities.file__feed_text_entities_descriptions()

        if file__feed_text_entities.exists__now() is False:
            with self.file_articles_current as _:
                _.load()
            ...
            for (article_id, file_to_process) in files_to_process__titles[0:self.max_articles_to_process]:
            ...
            for (article_id, file_to_process) in files_to_process__descriptions[0:self.max_articles_to_process]:
                json_data         = self.storage.path__load_data(file_to_process)
                mgraph_to_process = MGraph.from_json(json_data)
        ...
        self.path_now__text_entities                  = file__feed_text_entities             .path_now   ()
        self.path_now__text_entities__titles          = file__feed_text_entities_titles      .path_now   ()
        self.path_now__text_entities__descriptions    = file__feed_text_entities_descriptions.path_now   ()

    def task__3__move_articles_to_next_step(self):
        for article in self.articles_to_process[0:self.max_articles_to_process]:
    ...
    def task__4__create_output(self):
        self.output = dict(articles_to_process                      = len(self.articles_to_process),
                           ...
                           max_articles_to_process                  = self.max_articles_to_process,
