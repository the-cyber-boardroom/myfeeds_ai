...
FLOW__HACKER_NEWS__7__MAX__ARTICLES_TO_LOAD = 1
...
class Flow__Hacker_News__10__Article__Step_7__Create_Feed_Entities_MGraphs(Type_Safe):
    ...
    max_articles_to_load                     : int = FLOW__HACKER_NEWS__7__MAX__ARTICLES_TO_LOAD
    output                                   : dict
    ...
    def task__2__create_file_with_feed_text_entities_mgraph(self):
        ...
        for (article_id, file_to_process) in files_to_process__titles[0:self.max_articles_to_load]:
        ...
        for (article_id, file_to_process) in files_to_process__descriptions[0:self.max_articles_to_load]:
            json_data         = self.storage.path__load_data(file_to_process)
            mgraph_to_process = MGraph.from_json(json_data)
    ...
    def task__3__create_output(self):
        self.output = dict(articles_to_process                      = len(self.articles_to_process)                    ,
                           ...
                           max_articles_to_load                     = self.max_articles_to_load                        ,
